...
void Benchmark::loadProgram(const char * file_name)
{
...
void Benchmark::selectKernel(std::string kernel_name)
{
...
void Benchmark::setGridSize(int grid_size)
{
	m_global_range = cl::NDRange(grid_size, grid_size);
}

void Benchmark::setBlockSize(int block_size)
{
	m_local_range = cl::NDRange(block_size, block_size);
}

void Benchmark::performKernel(Matrix &A, Matrix &B, Matrix &C)
{
...
void Benchmark::printDeviceInfo()
{
...
float Benchmark::getProcessingTime()
{
